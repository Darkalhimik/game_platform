...
export function GameCatalogSortControls({
  ...
  const handleChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (isGameSortOption(nextValue)) {
      onChange(nextValue);
    }
  };

  return (
    <div className="relative inline-flex min-w-60 flex-col gap-1">
      <label htmlFor="games-sort-select" className="text-xs font-semibold uppercase tracking-wider text-white/60">
        Sort
      </label>
